  ...
  return (
    <Dialog open={open} TransitionComponent={FadeTransition}>
      <BoxWrapper sx={{ width: { xs: '100%', md: '290px' }, paddingTop: { xs: '30px', md: '20px' } }}>
        {!showReferral ?
          <Box>
            <Box><Typography variant='p_xlg' color='#b5fdf9'>Welcome to Clone!</Typography></Box>
          ...
          <Box height='100%' display='flex' flexDirection='column' justifyContent='center' alignItems='center'>
            {!doneReferral ?
                  ...
                  <OtpBox>
                    {isLinkingRefCode ?
                      <Box display='flex' justifyContent='center' alignItems='center' height='100%'>
                        <Box sx={{ width: '32px', height: '30px', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#1c1c1c', borderRadius: '10px', opacity: '0.85' }}><CircularProgress sx={{ color: '#fff' }} size={15} thickness={4} /></Box>
                      </Box>
                      :
                      <OtpInput
                        value={referralCode}
              ...
              :
              <Box><Typography variant='p_xlg' color='#b5fdf9'>Enjoy Clone!</Typography></Box>
            }
          </Box>
        }
...
const YesButton = styled(Box)`
  ...
  cursor: pointer;
  border-radius: 10px;
  background-color: ${(props) => props.theme.basis.liquidityBlue};
  &:hover {
