...
const CometBlank = ({ title, subtitle, icon, hoverIcon }: { title: string, subtitle: string, icon?: string, hoverIcon?: string }) => {
  ...
  return (
    <BoxWrapper>
      ...
      {icon && hoverIcon && <Link href='/comet/myliquidity'><IndicatorBox my="20px" onMouseOver={() => setIsBtnHover(true)} onMouseLeave={() => setIsBtnHover(false)}><Image src={isBtnHover ? hoverIcon : icon} alt={'icon'} /></IndicatorBox></Link>}
    </BoxWrapper>
  )
}
