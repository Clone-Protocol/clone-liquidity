...
import { CloneClient as Clone } from 'incept-protocol-sdk/sdk/src/clone'
import { getNetworkDetailsFromEnv } from 'incept-protocol-sdk/sdk/src/network'

export const useLoadedIncept = () => {
	const wallet = useAnchorWallet()
	const [incept, setIncept] = useState<Clone | undefined>(undefined)
	...
	useEffect(() => {
		const initializeIncept = async () => {
			...
			const CloneClient = new Clone(network.clone, provider)
